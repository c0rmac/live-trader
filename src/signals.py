def datapre(df):
    ...
    recent_df = df[df["time"] >= pd.Timestamp.now() - pd.Timedelta(days=90)]
    ...
    df['signal_total'] = df['signal_stoch'] + df['signal_obv'] + df['signal_cmf'] + df['signal_mae'] + df['signal_ichimoku'] + df['signal_fib'] + df['signal_bb'] + df['signal_macd'] + df['signal_rsi'] + df['signal_wt'] + df['signal_cci'] + df['signal_adx'] + df['signal_kc']
